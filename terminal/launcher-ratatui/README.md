...
- `n` (filter empty): **new project** popup — name, parent, template, init agent, multi-line notes; scaffolds a git repo then runs **headless init** via the chosen agent (harness-neutral recipes for Grok, Codex, Claude, Pi, Amp, Devin, Droid, Cursor). Notes: Enter inserts newline; create with the **Create** row, or **Ctrl+Enter (in T-0’s browser terminal)** — Terminal.app may not deliver Ctrl+Enter. In long notes, **Shift+Up** leaves the notes field without scrolling. Word/line delete: **Opt+Backspace** / **Ctrl+W** (word), **Ctrl+U** (line). Agents with elevated autonomy show **full tools** on Create/Init.
